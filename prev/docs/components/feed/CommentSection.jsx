    ...
    <div className="mt-4 pt-4 border-t border-gray-100">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 mb-4">
        <Avatar className="h-8 w-8 flex-shrink-0">
          <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${currentUser?.full_name}`} />
          ...
          {comments.map((comment, index) => (
            ...
            >
              <Link to={`${createPageUrl('Profile')}?email=${comment.author_email}`}>
                <Avatar className="h-8 w-8 flex-shrink-0 cursor-pointer hover:ring-2 hover:ring-red-200 transition-all">
              ...
              <div className="flex-1 bg-gray-50 rounded-2xl px-3 py-2">
                <div className="flex items-center gap-2">
                  <Link 
                    ...
                    className="font-semibold text-sm hover:text-red-600 transition-colors"
                  >
                    {comment.author_name}
                  </Link>
                  <span className="text-sm">{tierIcons[comment.author_tier]}</span>
                </div>
